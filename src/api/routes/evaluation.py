...
async def run_full_evaluation_engine(user_id: int, task_id: int):
    ...
    from api.engine.llm_evaluator import run_llm_evaluation
    ...
    from api.engine.confidence_gate import compute_tier1_weighted_score, should_run_llm
    import json
    ...
    q_rows = await execute_db_operation(
        f"SELECT id, title, answer, blocks FROM {questions_table_name} WHERE task_id = ? AND deleted_at IS NULL ORDER BY position",
    ...
    async def generate():
        ...
        for idx, q_row in enumerate(q_rows):
            q_id, q_title, q_answer_raw, q_blocks_raw = q_row
